...
function listFolders() {
  ...
  while(contents.hasNext()) {
    ...
    if(file.getName() != 'releases') {
      var row = {
        ...
        'md5sha1' : getPassword(file.getId()),
      };
      data[i] = row;
      i++;
    }
  ...
  // var sFolders = DriveApp.searchFolders('sharedWithMe');
  var sContents = DriveApp.searchFolders('sharedWithMe  and ("me" in owners or "me" in writers)');
  while(sContents.hasNext()) {
    var file = sContents.next();
...
function getPrivateKeyPem() {
  ...
  while(contents.hasNext()) {
    var file = contents.next();
    if (file.getName() == 'privatekey.pem' 
...
function encrypt(text) {
  ...
  var str = text;
  for(var i = 0; i < 5; i++) {
    // for now just use Base64 encoding 5X
    var blob = Utilities.newBlob(str);
    str = Utilities.base64Encode(blob.getBytes());
...
function getPassword(folderId) {
  ...
  var pdoc = DocumentApp.create('password'),
  docFile = DriveApp.getFileById( pdoc.getId() ); 
...
function createSheet(params) {
  ...
  if(param['folderId'] == null || param['name'].length == 0) {
    throw new Error('SangyawApp API Error: folder ID and Sheet Name is required!');
  }
  var sheetName = param['name'];
  var folderId = param['folderId'];
  var folder = getFolder(folderId);
  if(folder == null) {
    throw new Error('SangyawApp API Error: folder does not exists');
  }
  ...
  while(contents.hasNext()) {
    ...
    if (file.getName() == sheetName
      ...
      throw new Error('SangyawApp API Error: Folder already exists');      
    }
  }


  var sheet = SpreadsheetApp.create(sheetName);
  var docFile = DriveApp.getFileById(sheet.getId()); 
  DriveApp.getFolderById(folderId).addFile( docFile );
  DriveApp.getRootFolder().removeFile(docFile);
  sheet.renameActiveSheet('MasterList');
  var row = ['Facebook Name',	
              'Gender',
              'Address',
              'Age Group',
              'Messenger Status',
              ...
              'RecordStatus'];
  sheet.appendRow(row);
  return sendData({ 'created': true, 'sheetId': sheet.getId(), 'folderId': folderId});
}

...
function saveSheetValuesAtRow(sheet, row, params ) {

    var person = params['parameter'];
    var rowData = [];
    rowData[0] = person['Facebook Name'];
    rowData[1] = person['Gender'];
    rowData[2] = person['Address'];
    rowData[3] = person['Age Group'];
    rowData[4] = person['Messenger Status'];
    rowData[5] = person['Profile Image'];
    rowData[6] = person['Reference Details'];
    rowData[7] = person['Assigned To'];
    rowData[8] = person['Preached By'];
    rowData[9] = person['Date Contacted'];
    rowData[10] = person['Remarks'];
    rowData[11] = person['Progress Status'];
    sheet.getRange(row, 1, 1, rowData.length).setValues([rowData]);

    var data = { 'completed': true, 'row': row };
    return sendData(data);
...
function insertPerson(params) {
  ...
  return saveSheetValuesAtRow(sheet, row, params );
}
...
function savePerson(params) {
  ...
  var row = findPersonRow(sheet, params['Facebook Name']);
  if(row != -1 ) {
    return saveSheetValuesAtRow(sheet, row, params );
  }
...
function findPersonRow(sheet, fbName) {
  ...
  var range = sheet.getRange('A1:A' + row);
  var values = range.getValues();

  var searchResult = columnValues.findIndex(fbName); //Row Index - 2
